...
router.post("/RelativeSongs", function (request, response) {
  ...
  let sql = "select song_id, song_title, album_id from song_info where song_id in (select C.song_id from artist_info A inner join artist_song C on A.artist_id = C.artist_id where A.artist_id = (select A.artist_id from song_info S inner join artist_song C on S.song_id = C.song_id inner join artist_info A on C.artist_id = A.artist_id where S.song_id = ?))";
  conn.query(sql, [song_id], function (err, rows) {
    if (rows) {
      console.log("연관 곡 불러오기 성공!")
      console.log(rows.length);
      let ranNum = [];
      let j=0;
      while(j<rows.length){
      ...
      for(let i=0; i<3; i++){
        song_list.push(rows[ranNum[i]].song_id);
        stitle_list.push(rows[ranNum[i]].song_title);
        salbum_list.push(rows[ranNum[i]].album_id);
        console.log(rows[ranNum[i]].song_id);
        console.log(rows[ranNum[i]].song_title);
        console.log(rows[ranNum[i]].album_id);
      }
      console.log(song_list);
      console.log(stitle_list);
      console.log(salbum_list);
